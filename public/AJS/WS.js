  ...
  function runSQLTransaction(obj) {
    ...
    if (!obj.sqlList || obj.sqlList.length == 0) {
      ...
      try {
        ...
        strSql += "SELECT ERROR_NUMBER() AS ErrorNumber, ERROR_SEVERITY() AS ErrorSeverity, ERROR_STATE() AS ErrorState, ERROR_PROCEDURE() AS ErrorProcedure, ERROR_LINE() AS ErrorLine, ERROR_MESSAGE() AS ErrorMessage;"
        strSql += "ROLLBACK;";
        strSql += "END";
        strSql += "END CATCH;";
        X.RUNSQL(strSql);
