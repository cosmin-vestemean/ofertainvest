    ...
    for (let i = 0; i < r.length; i++) {
      ...
      if (r[i].hasChildren) {
        ...
        for (let k = 0; k < m.length; k++) {
          ...
          let sql =
            `insert into CCCMATRETETE (CCCOFERTEWEB, CCCRETETE, CCCACTIVITRETETE, CCCOFERTEWEBLINII, PONDEREDECONT, PONDERENORMA, ISCUSTOM, CANTITATEUNITARA) ` +
            `values (${contextOferta.CCCOFERTEWEB}, ${max + j}, ${maxActivitati + i}, ${CCCOFERTEWEBLINII.value}, ${material.PONDERE_DECONT_ACTIVITATE_ARTICOL_RETETA || 0}, ${material.PONDERE_NORMA_ACTIVITATE_ARTICOL_RETETA || 0}, 0, ${material.CANTITATE_UNITARA_ARTICOL_RETETA || 0})`
          sqlList.push(sql)
        }
      }
    }
