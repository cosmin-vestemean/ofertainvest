...
export async function salveazaReteteInDB() {
  ...
  recipes_ds.forEach((reteta) => {
    let sql = `insert into CCCRETETE (CCCOFERTEWEB, NAME, ID, TYPE) values (${contextOferta.CCCOFERTEWEB}, '${reteta.name}', ${reteta.id}, ${reteta.reteta[0].type || 'null'})`
    sqlList.push(sql)
  })
    ...
    for (let i = 0; i < r.length; i++) {
      ...
      if (r[i].hasChildren) {
        ...
        for (let k = 0; k < m.length; k++) {
          ...
          let sql =
            `insert into CCCMATRETETE (CCCOFERTEWEB, CCCRETETE, CCCACTIVITRETETE, CCCOFERTEWEBLINII, CANTUNIT, CANTREAL, ISCUSTOM, CANTTOTAL, CANTREALUNIT, NORMUNITMAN, TOTALOREMAN, PONNORMMAN, ISCUSTOM) ` +
            `values (${contextOferta.CCCOFERTEWEB}, ${max + j}, ${maxActivitati + i}, ${CCCOFERTEWEBLINII.value}, ${material.CANTITATE_UNITARA_SUBARTICOL_RETETA || 0}, ${material.CANTITATE_REALIZARE_ARTICOL_RETETA || 0}, 0, ${material.CANTITATE_SUBARTICOL_RETETA || 0} , ${material.CANTITATE_REALIZARE_UNITARA_ARTICOL_RETETA || 0}, ${material.NORMA_UNITARA_ORE_MANOPERA_ARTICOL_OFERTA || 0}, ${material.SUMA_TOTAL_ORE_MANOPERA_ARTICOL_OFERTA || 0}, ${material.PONDERE_NORMA_ACTIVITATE_ARTICOL_RETETA || 0}, 0)`
          sqlList.push(sql)
        }
      }
    }
